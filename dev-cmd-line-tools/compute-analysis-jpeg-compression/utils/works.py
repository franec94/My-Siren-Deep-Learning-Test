    ...
    for quality in qualities:
        try:
            ...
            with BytesIO() as f:
                ...
                psnr_score = psnr(np.asarray(image).astype(dtype = np.uint8), np.asarray(im_jpeg).astype(dtype = np.uint8), data_range=255)
                ssim_score = ssim(np.asarray(image).astype(dtype = np.uint8), np.asarray(im_jpeg).astype(dtype = np.uint8), data_range=255)
