...
def _evaluate_model_local(image_dataset, model_conf, quant_tech = None, device = 'cpu', num_bits = 8, quant_sym = False, dtype = torch.qint8):
    ...
    if quant_tech == None:
        ...
        if model_conf.quantization_enabled.lower() == 'custom_quant':
            quant = True
            arch_hyperparams['quant'] = quant
            arch_hyperparams['num_bits'] = num_bits
            arch_hyperparams['quant_sym'] = quant_sym
            pass
        if model_conf.quantization_enabled.lower() == 'dynamic':
        ...
        for a_tech in quant_tech_list:
            ...
            if a_tech == 'dynamic':
                for dtype in args.dynamic_quant:
                    ...
                    vals_r = [os.path.basename(vals.path), int(vals.hl), int(vals.hf), opt.sidelength, f"{a_tech}-{str(dtype)}"] + list(eval_scores) + [eta_eval, size_model]
                    a_record = InfoResults._make(vals_r)
                    records_list.append(a_record)
            else:
                eval_scores, eta_eval, size_model = _evaluate_model_local(
